...
class PrayerRepository(private val ctx: Context) {
    ...
    private val K_MEMORIZED_JUZ = stringPreferencesKey("memorized_juz_map")
    private val K_READING_LAST_DATE = longPreferencesKey("reading_last_date")
    private val K_READING_STREAK = intPreferencesKey("reading_streak")
    ...
    private suspend fun parseMemorizedMap(): MutableMap<Int, Int> {
        val raw = ctx.dataStore.data.first()[K_MEMORIZED_JUZ] ?: ""
        val out = mutableMapOf<Int, Int>()
        for (entry in raw.split(",")) {
            val kv = entry.split("=")
            if (kv.size == 2) {
                val k = kv[0].toIntOrNull()
                val v = kv[1].toIntOrNull()
    ...
    suspend fun getJuzProgress(juz: Int): Pair<Int, Int> {
        val parts = parseMemorizedMap()
        val total = if (juz in 1..30) JUZ_VERSE_COUNTS[juz - 1] else 0
    ...
    suspend fun setJuzMemorized(juz: Int, count: Int) {
        val parts = parseMemorizedMap()
        parts[juz] = count
        val joined = parts.entries.joinToString(",") { e -> "${e.key}=${e.value}" }
        ctx.dataStore.edit { prefs -> prefs[K_MEMORIZED_JUZ] = joined }
    }

    suspend fun getMemorizedCount(): Int {
        return parseMemorizedMap().values.sum()
    }
    ...
    suspend fun markReadToday() {
        ...
        val newStreak = when {
            daysSince == 0L -> streak
            daysSince == 1L -> streak + 1
            else -> 1
        }
        ctx.dataStore.edit { prefs ->
            prefs[K_READING_STREAK] = newStreak
            prefs[K_READING_LAST_DATE] = now
        }
    }
    ...
    suspend fun getNextPrayer(): String? {
        ...
        val nextEntry = prayers.firstOrNull { it.second > nowMin }
        if (nextEntry == null) {
            return "Fajr at ${minToHm(prayers[0].second)} tomorrow"
        }
        val deltaMin = nextEntry.second - nowMin
        val hm = "${deltaMin / 60}h ${deltaMin % 60}m"
        return "${nextEntry.first} in $hm  -  ${minToHm(nextEntry.second)}"
    }
